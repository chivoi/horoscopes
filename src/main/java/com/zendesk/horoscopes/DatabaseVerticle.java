...
public class DatabaseVerticle extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) {
    pgPool = PgPool.pool(vertx, new PgConnectOptions().setHost("127.0.0.1").setUser("agoh").setDatabase("postgres").setPassword("vertx-in-action"), new PoolOptions());
...
//    router.get("/temperature").handler(this::getTemperatureData);
  ...
  private void recordHoroscope(Message<JsonObject> message) {
    ...
    pgPool.preparedQuery("insert into horoscopes(date, compatability, color, luckynumber, description) values($1, $2, $3, $4, $5)").execute(Tuple.of(date, compatability, color, luckynumber, description), ar -> {
      if (ar.succeeded()) {
  ...
  public void getHoroscopeData(RoutingContext context) {
    ...
    pgPool.preparedQuery(query).execute().onSuccess(rows -> {
      ...
      for (Row row : rows) {
        array.add(new JsonObject().put("date", row.getString("date")).put("compatibility", row.getString("compatability")).put("color", row.getString("color")).put("luckynumber", row.getString("luckynumber")).put("description", row.getString("description")));
      }
      context.response().putHeader("Content-Type", "application/json").end(new JsonObject().put("data", array).encode());
    }).onFailure(failure -> {
...
//      JsonArray array = new JsonArray();
//      for (Row row : rows) {
//        array.add(new JsonObject().put("uuid", row.getString("uuid")).put("temperature", row.getDouble("value")).put("timestamp", row.getTemporal("tstamp").toString()));
//      }
//      context.response().putHeader("Content-Type", "application/json").end(new JsonObject().put("data", array).encode());
//    }).onFailure(failure -> {
//      logger.info("Whoops: " + failure);
...
//
//    pgPool.preparedQuery("insert into postgres2(uuid, value, tstamp) values($1, $2, $3)").execute(Tuple.of(uuid, temperature, tstamp.toLocalDateTime()), ar -> {
//      if (ar.succeeded()) {
//        RowSet<Row> rows = ar.result();
